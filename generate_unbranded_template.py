...
def generate_first_page(doc, data=None):
    ...
    spacer.paragraph_format.space_after = Inches(5)
    ...
    run = name_para.add_run(f"CV OF: {name_val}")
    run.font.name, run.font.size, run.font.bold = 'Arial', Pt(28), True
    ...
    run.font.name, run.font.size, run.font.bold = 'Arial', Pt(22), True
    ...
    run.font.name, run.font.size, run.font.bold = 'Arial', Pt(11), True
    ...
    style_utils.apply_table_styles(table)
    table.alignment = WD_ALIGN_PARAGRAPH.CENTER
    table.width, table.rows[0].height = Inches(3.3), Inches(1.7)
    ...
    def add_centered(text, bold=True):
        ...
        r = p.runs[0]
        r.font.name, r.font.size, r.font.bold = 'Arial', Pt(9), bold
...
def add_personal_details(doc, data):
    ...
    fields = [
        ("Full Name", "CANDIDATE_NAME"), ("Known As", "KNOWN_AS"), ("Race and Gender", "RACE_GENDER"),
        ("Nationality", "NATIONALITY"), ("Marital Status", "MARITAL_STATUS"), ("Date of birth", "DOB"),
        ("Driver's License / Own Car", "DRIVERS"), ("Notice Period", "NOTICE"),
        ("Do you have clear criminal record", "CRIMINAL"), ("Area of residence", "RESIDENCE"),
        ("Languages", "LANGUAGES"), ("Salary Expectations", "SALARY_EXP"), ("Skills", "SKILLS"),
        ("Computer literacy", "COMPUTER"), ("Experience", "EXPERIENCE_SUMMARY")
    ]
...
def add_projects(doc, data):
    ...
    for proj in projects:
        ...
        p_title.paragraph_format.left_indent, p_title.paragraph_format.space_after = Inches(0.25), Pt(0)
        ...
        p_value.paragraph_format.left_indent, p_value.paragraph_format.space_after = Inches(0.25), Pt(0)
        for b in bullets:
...
def generate_cv(data, output_path, section_order=None, layout=None):
    ...
    if not section_order:
        section_order = ["Personal Details", "Courses", "Qualification", "Professional Membership", "Projects", 
                         "Course Work", "Training", "Career Highlights", "Career Summary", "Career History"]
    for section_name in section_order:
        if section_name in SECTION_MAP:
            if is_section_populated(data, section_name): SECTION_MAP[section_name](doc, data)
        else: print(f"Warning: Section '{section_name}' not found.")
    ...
    print(f"Unbranded CV generated: {output_path}")

def create_template():
    output_file = "uploads/Unbranded_CV_Template.docx"
    generate_cv({}, output_file)
    print(f"Unbranded Template created: {output_file}")
